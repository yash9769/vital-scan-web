...
export function predictDiabetesRisk(data: HealthData): RiskResult {
  // Check if a trained model is available
  const savedModel = localStorage.getItem('diabetesModel');
  const modelAccuracy = localStorage.getItem('modelAccuracy');
  
  if (savedModel) {
    try {
      return useTrainedModel(data, JSON.parse(savedModel), modelAccuracy);
    } catch (error) {
      console.warn('Error using trained model, falling back to rule-based:', error);
    }
  }
...
function useTrainedModel(data: HealthData, model: { weights: number[]; bias: number }, accuracy: string | null): RiskResult {
  ...
  const dietMap = { 'Unhealthy': 0, 'Balanced': 1, 'Healthy': 2 };
  const smokingMap = { 'Smoker': 2, 'Former Smoker': 1, 'Non-Smoker': 0 };
  const alcoholMap = { 'Yes': 2, 'Occasional': 1, 'No': 0 };
  const stressMap = { 'High': 2, 'Medium': 1, 'Low': 0 };

  const features = [
    data.age,
    genderMap[data.gender as keyof typeof genderMap] ?? 0,
    data.bmi,
    binaryMap[data.familyHistory as keyof typeof binaryMap] ?? 0,
    activityMap[data.physicalActivity as keyof typeof activityMap] ?? 0,
    dietMap[data.dietType as keyof typeof dietMap] ?? 0,
    smokingMap[data.smokingStatus as keyof typeof smokingMap] ?? 0,
    alcoholMap[data.alcoholIntake as keyof typeof alcoholMap] ?? 0,
    stressMap[data.stressLevel as keyof typeof stressMap] ?? 0,
    binaryMap[data.hypertension as keyof typeof binaryMap] ?? 0
  ];
